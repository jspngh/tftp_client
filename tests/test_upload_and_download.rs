...
use tftp_client::{download_blocking, upload_blocking};
...
fn download_upload_blocking() {
    ...
    .unwrap();
    let res = download_blocking("/test", &socket, server, timeout, max_timeout, retries).unwrap();
    assert_eq!(test_payload, res);
}
